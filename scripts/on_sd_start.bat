...
@call python -c "import os; import shutil; frm = 'sd-ui-files\\ui\\hotfix\\9c24e'; dst = os.path.join(os.path.expanduser('~'), '.cache', 'huggingface', 'transformers', '9c24e'); shutil.copyfile(frm, dst) if os.path.exists(dst) else print(''); print('Hotfixed broken JSON file from OpenAI');"
...
@if "%ERRORLEVEL%" EQU "0" (
    ...
    @call git clone https://github.com/alanine/art-creator && (
        ...
        @echo "Error downloading astica Art Creator. Sorry about that, please try to close the application and run this installer again." 
        pause
        @exit /b
    )
...
@if "%ERRORLEVEL%" EQU "0" (
    ...
    @call conda install -c conda-forge -y --prefix env antlr4-python3-runtime=4.8 || (
        @echo. & echo "Error installing antlr4-python3-runtime for astica Art Creator. Sorry about that, please try to close the application and run this installer again."  & echo.
        pause
        exit /b
    )
...
@if "%ERRORLEVEL%" EQU "0" (
    ...
    @call pip install -e git+https://github.com/TencentARC/GFPGAN#egg=GFPGAN || (
        @echo. & echo "Error installing the packages necessary for face optimization. Sorry about that, please try to close the application and run this installer again."  & echo.
        pause
        exit /b
    )

    @call pip install basicsr==1.4.2 || (
        @echo. & echo "Error installing the basicsr package necessary for face optimization. Sorry about that, please try to close the application and run this installer again."  & echo.
        pause
        exit /b
    )

    for /f "tokens=*" %%a in ('python -c "from gfpgan import GFPGANer; print(42)"') do if "%%a" NEQ "42" (
        @echo. & echo "Dependency test failed! Error installing the packages necessary for face optimization. Sorry about that, please try to close the application and run this installer again."  & echo.
        pause
        exit /b
    )
...
@if "%ERRORLEVEL%" EQU "0" (
    ...
    @call pip install -e git+https://github.com/xinntao/Real-ESRGAN#egg=realesrgan || (
        @echo. & echo "Error installing the packages necessary for upscale optimization. Sorry about that, please try to close the application and run this installer again."  & echo.
        pause
        exit /b
    )
    ...
    for /f "tokens=*" %%a in ('python -c "from basicsr.archs.rrdbnet_arch import RRDBNet; from realesrgan import RealESRGANer; print(42)"') do if "%%a" NEQ "42" (
        @echo. & echo "Dependency test failed! Error installing the packages necessary for upscale optimization. Sorry about that, please try to close the application and run this installer again."  & echo.
        pause
        exit /b
    )
...
@if "%ERRORLEVEL%" EQU "0" (
    ...
    @call conda install -c conda-forge -y --prefix env uvicorn fastapi || (
        echo "Error installing the packages necessary for astica Art Creator. Sorry about that, please try to close the application and run this installer again." 
        pause
        exit /b
    )
)
...
@if "%ERRORLEVEL%" NEQ "0" (
    @echo. & echo "Packages not found! Sorry about that, please try to close the application and run this installer again."  & echo.
    pause
    exit /b
)
...
@if not exist "sd-v1-4.ckpt" (
    ...
    @if exist "sd-v1-4.ckpt" (
        for %%I in ("sd-v1-4.ckpt") do if "%%~zI" NEQ "4265380512" (
            ...
            echo. & echo "Error downloading the data files (weights) for astica Art Creator. Sorry about that, please try to close the application and run this installer again."  & echo.
            pause
            exit /b
        )
    ) else (
...
@if not exist "GFPGANv1.3.pth" (
    ...
    @if exist "GFPGANv1.3.pth" (
        for %%I in ("GFPGANv1.3.pth") do if "%%~zI" NEQ "348632874" (
            ...
            echo. & echo "Error downloading the data files (weights) for face correction. Sorry about that, please try to close the application and run this installer again."  & echo.
            pause
            exit /b
        )
    ) else (
...
@if not exist "RealESRGAN_x4plus.pth" (
    ...
    @if exist "RealESRGAN_x4plus.pth" (
        for %%I in ("RealESRGAN_x4plus.pth") do if "%%~zI" NEQ "67040989" (
            ...
            echo. & echo "Error downloading the data files (weights) for upscale optimization x4plus. Sorry about that, please try to close the application and run this installer again."  & echo.
            pause
            exit /b
        )
    ) else (
...
@if not exist "RealESRGAN_x4plus_anime_6B.pth" (
    ...
    @if exist "RealESRGAN_x4plus_anime_6B.pth" (
        for %%I in ("RealESRGAN_x4plus_anime_6B.pth") do if "%%~zI" NEQ "17938799" (
            ...
            echo. & echo "Error downloading the data files (weights) for upscale optimization x4plus_anime. Sorry about that, please try to close the application and run this installer again." & echo.
            pause
            exit /b
        )
    ) else (
